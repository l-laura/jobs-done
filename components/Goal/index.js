...
import { qsGoal, stepStateType } from '../shared/propTypes';
import { Transition, QUICK_TRANS_TIME } from '../shared/Transition';
import { Checkbox } from './Checkbox';
import { Link } from './Link';
import { shuffle } from 'lodash';

export class Goal extends UnmountAwareComponent {
  static propTypes = {
    goal: qsGoal.isRequired,
  ...
  pickGoalForm(forms) {
      console.log(forms)
      var mixedForms = shuffle(forms);
      return mixedForms.pop();
  }

  renderGoal(bgOpacity) {
    const {
      goal,
      state,
      mobileViewport,
    ...
    const goalForm = this.pickGoalForm(goal.forms)
    ...
    return (
      ...
      >
        <Left>
          <Name>{isDone ? goalForm.whenDone : goalForm.whenSet}</Name>
